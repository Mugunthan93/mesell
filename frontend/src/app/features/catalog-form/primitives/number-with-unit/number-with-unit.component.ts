// features/catalog-form/primitives/number-with-unit/number-with-unit.component.ts
// Primitive 4: numeric input with unit suffix per §18.E
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => NumberWithUnitPrimitiveComponent),
      multi: true,
    },
  ],
  template: `
    <mat-form-field appearance="outline" class="w-full">
      ...
      <input
        ...
        (blur)="onBlur()"
      />
      @if (schema().displayUnitLabel) {
        <span matSuffix>{{ schema().displayUnitLabel | localeLabel }}</span>
      }
...
export class NumberWithUnitPrimitiveComponent implements ControlValueAccessor {
