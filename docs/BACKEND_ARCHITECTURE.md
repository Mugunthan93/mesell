...
> **AMENDMENT — founder-ratified 2026-06-19 (Razorpay Wave 4).** The "exactly 2 task modules" inventory is widened to **3**: `modules/iam/tasks.py` is added as the 3rd module-owned `tasks.py`, registered via `celery_app.py` `include=["app.modules.image.tasks", "app.modules.export.tasks", "app.modules.iam.tasks"]`. It hosts the two billing **periodic** sweeps `billing.reconcile` (every 6 h) + `billing.trial_expiry_sweep` (daily 02:00 IST). This also introduces the repo's **first `beat_schedule`** (in `celery_app.py`, the only module authorised to mutate the Celery app config). The `iam` module owns billing/subscription state, so the sweep task bodies belong to it per the §3.C modular-monolith discipline. The beat *process* itself (`celery … beat`) is an infra deployment, NOT this file (see §18 INFRA hand-off). No other §3.I rule changes — task definitions still live in the owning module's `tasks.py`; cross-module enqueue is still by task-name string.
...
<!-- AS-BUILT SENTINEL 2026-06-11: §7 iam code is on-disk and verified. Merged to develop via PR #46 (feature/auth-otp/integration → develop, merge SHA cad0a9a) = backend group PR #44 + infra group PR #45. The 6 iam endpoint contracts, verbatim Lua rotation script (§7.B.3), FE-D5 split-token mechanics, and 8 IamError subclasses are implemented as locked. See BACKEND_VERIFICATION.md and V1_FEATURE_SPEC.md Feature 1 AS-BUILT note. Additive sentinel only — no §7 contract change. -->

### 7.A Preamble

§7 specifies the **`iam` module** — Identity & Access Management. **Owner specialist:** `meesell-auth-builder` (sole owner per §2.1 + §4.B). **Leaf module** — `iam` calls no other module's `service.py`; other modules consume `get_current_user` through the `core/` middleware chain, not by service-layer call (per §2.D matrix all-`✗` row for `iam`). Owns the `users` table **exclusively** — no other module writes to it. Surfaces **6 endpoints total** — 4 V1 auth endpoints (counted in the §0.C 27-endpoint contract) + 1 introspection (`GET /api/v1/auth/me`) + 1 webhook (`POST /api/v1/webhooks/razorpay`). The `/me` and `/webhooks/razorpay` surfaces are **infrastructure** surfaces — NOT counted in the §0.C contract; the contract is the 4 auth endpoints. **[AMENDMENT 2026-06-18 — see §7.3 below: the google-auth dual-identity decision adds a 5th V1 auth endpoint `POST /api/v1/auth/google/verify`, raising the iam endpoint count 6 → 7 (5 auth + `/me` + webhook) and the §17 mounted total 28 → 29. The new route is feature-flag gated (`FEATURE_GOOGLE_AUTH_ENABLED`, default `False`) and is NOT mounted when the flag is off — so both counts remain 6/28 until the flag is enabled per namespace.]** §7 does NOT specify `core/auth.py`'s `get_current_user` (that is §4.B — `iam` re-exports it via FastAPI router-level `Depends` only); does NOT specify rate-limit Valkey window math (that is §4.G); does NOT specify the JWT claim shape (that is §4.B + `MVP_ARCH §11.7` — `iam` writes the claims defined there). A reviewer evaluating §7 asks: "are the 6 endpoint contracts unambiguous (request, response, cookie attributes, audit events, status codes), is the Lua rotation script verbatim, are the service/repository surfaces locked and module-private per §16, do the 8 IamError subclasses cover every documented failure mode?" — not "is the get_current_user dep correct" (§4.B) or "is the MSG91 SDK call right" (§6.C).

### 7.3 AMENDMENT — google-auth dual-identity (FOUNDER-RATIFIED 2026-06-18)

> **Founder-ratified amendment to a LOCKED section** (per repo management master plan §7.3 / Key Decision #5). This block records the architecture impact of the dual-identity (phone OR Google) decision. It is consistent with the CLAUDE.md Key Decision #5 amendment (2026-06-18) and the design rationale in `docs/plans/google_auth/GOOGLE_AUTH_DESIGN_BACKEND.md §A`. Reverse this amendment by deleting the `POST /api/v1/auth/google/verify` route + the flag-gated mount and restoring the 6/28 counts.

**What changes:**

1. **Key Decision #5 widened to DUAL-IDENTITY.** A user may exist with phone only (every existing row), Google only (`email` + Google `sub`, `phone = NULL`), or both. `iam.users.phone` becomes nullable-unique; `email` gains a nullable-UNIQUE linking constraint; a new nullable-UNIQUE `google_sub` column and an `auth_provider` audit column are added; a table-level CHECK guarantees `phone IS NOT NULL OR google_sub IS NOT NULL`. The phone-OTP path is unchanged. (Migration `c2d3e4f5a6b7`, additive + widening only.)
2. **iam endpoint count 6 → 7.** A new V1 auth endpoint `POST /api/v1/auth/google/verify` is added to the `iam` module (the 7th surface = 5 auth + `/me` + `/webhooks/razorpay`). Request `GoogleVerifyRequest {credential}`; response = the existing `VerifyOtpResponse` shape (declared as a thin alias `GoogleVerifyResponse`) + the `Set-Cookie: refresh_token=...` header via the existing `_set_refresh_cookie` helper — byte-identical cookie attributes to the OTP path. No change to FE-D5 (§4.B): Google's ID-token is consumed once at verify, never stored, never placed in a cookie.
3. **§17 mounted-endpoint inventory 28 → 29.** The locked operational mount count rises from 28 to 29 routes on `app/main.py`.
4. **Flag gating — counts stay 28 until the flag is flipped.** The new route is mounted ONLY when `FEATURE_GOOGLE_AUTH_ENABLED` (default `False`) is true. When the flag is off, the route is not registered, the OpenAPI surface is unchanged, and the §17 mounted total remains **28**. The 29-count applies per namespace once the flag is enabled (dev → staging; prod deferred to V1.5). Therefore the locked §17.G assertion (`28 routes mounted`) remains correct in the flag-off default state and increments to **29** only in flag-on namespaces.
5. **Adapters — no cross-MODULE matrix change.** A 6th adapter `adapters/google.py` is added, consumed by `iam.service` (extends the §6.G/§19 import-linter adapter allowlist exactly as `iam` already consumes `adapters.msg91` / `adapters.razorpay`). Adapters are not modules, so the §2.D cross-module call matrix is unchanged — no new ✗ → ✓.

### 7.B Endpoint surfaces

The 6 endpoint contracts owned by `iam` (**7 when `FEATURE_GOOGLE_AUTH_ENABLED` is on — see §7.3**). Each block locks request shape, response shape, rate-limit decorator (per §4.G/§4.H decorator pattern), status code → `validation_message_id` mapping (per §5A.H), audit event emission (middleware-default with documented exceptions), and the service-layer flow.
...
**28 routes mounted on `app/main.py` after construction** (**29 when `FEATURE_GOOGLE_AUTH_ENABLED` is on per §7.3 amendment 2026-06-18** — the flag-gated `POST /api/v1/auth/google/verify` is the 29th; with the flag off, default, the count stays 28). This is the assertion the §19 boot integration tests verify (`tests/test_app_boot_integration.py` currently passes at 7/7 with 9 routes; post-construction the assertion increments to 34 routes when accounting for `/health` + FastAPI defaults per §17.B.3).
...
| `billing.reconcile` | `iam` | `meesell-services-builder` | Razorpay Wave 4 (`WAVE4_RECONCILIATION_TASKSPEC.md`) | 0 (next beat re-runs) | n/a — `beat_schedule` every 6 h | Yes — converges local subs to Razorpay truth via the SAME idempotent/monotonic webhook guards; no-op when already converged; per-sub isolation |
| `billing.trial_expiry_sweep` | `iam` | `meesell-services-builder` | Razorpay Wave 4 (`WAVE4_RECONCILIATION_TASKSPEC.md`) | 0 (next beat re-runs) | n/a — `beat_schedule` daily 02:00 IST | Yes — `billing.trial.expired` audit-row marker makes the sweep fire once per user; `trial_ends_at` deliberately NOT nulled (R5: would re-open the once-per-phone trial) |

**§18.B.1 The task floor — AMENDED founder-ratified 2026-06-19 (Razorpay Wave 4): the V1 2-task floor is widened to 4 (the 2 V1 request-driven tasks + the 2 Wave-4 periodic billing sweeps).** The 2 V1 tasks (`image.precheck`, `export.xlsx`) remain the only *request-driven* enqueues. The `ai_ops` layer's Gemini calls are synchronous (await within the request handler) — they do NOT enqueue. The audit_events writes are synchronous inline per §4.G `audit_mw`. The 2 Wave-4 tasks are **periodic** (Celery `beat_schedule`, NOT request-enqueued); they live in `modules/iam/tasks.py` (§3.I amendment 2026-06-19) and are exempt from the §18.F `task_prerun` user-revalidation handler (they take no per-user args — they sweep their own work set). The `beat_schedule` is the repo's first; the `celery … beat` process is an infra deployment (see §18 INFRA hand-off), guarded against accidental double-fire by a Valkey DB-0 singleton lock per task.
