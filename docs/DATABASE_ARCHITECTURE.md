...
This makes the monthly, usage-driven Meesho refresh safe: re-run `seed_all.py` after updating batch JSONs.
...
```
**Must be fresh:** If `build_template_schemas.py` is re-run (e.g. after a monthly refresh),
...
| Monthly, usage-driven Meesho refresh | DOCUMENTED | Re-seed is idempotent. Schema changes in the refresh require manual review + new migration; no auto-migration. |
