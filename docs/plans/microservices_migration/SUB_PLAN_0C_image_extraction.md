...
- **CROSS-SCHEMA READ HAZARD (Risk #5-class) — RULED OPTION B (founder 2026-06-13):** the tenancy join reads `products` (catalog-owned, schema `catalog` after MS-5). At MS-C time catalog is STILL in `public`/in-process, so the join works in-process. But svc-image is being extracted to schema `image` while it must read `products`. **This is the one structural wrinkle vs export** (export owns its read path). The MS-A recipe did NOT cover this (export owns its read path), so the founder ruled a NEW precedent: **resolve image-service tenancy via the catalog ownership HTTP shim, NOT a cross-schema DB read-grant.** Rationale (founder, verbatim): matches the LOCKED §2.D principle ("no cross-schema reads — services talk via HTTP, never SQL"); no transitional debt to unwind at MS-5; avoids two services sharing one DB.
  - **EXECUTION (RULED):** the MS-C builders scope the repository by `product_id` alone (using the `assert_product_ownership` shim result, which already proves ownership). The repository writes **ZERO cross-schema SQL** — the `products` join is removed, not read-granted. The `meesell-database-builder` grants **NO `products` read access** to `image_user`. The two prior resolution options [(a) transitional `public.products` read grant; (b) shim-scoped repository] are SUPERSEDED — Option B (the shim-scoped repository) is now the locked target; the read-grant path (Option A) is DISCARDED, not a fallback.
...
| → `meesell-database-builder` (sonnet) | Alembic schema-split moving `product_images` from `public` to schema `image`; `version_table_schema="image"`; Risk#5 integrity pre-scan. **§0.10 RULED OPTION B (founder 2026-06-13): grant NO `products` read access to `image_user`** — the repository scopes by `product_id` via the ownership shim, so there is NO cross-schema read to grant. |
...
PHASE A (parallel — no inter-dependency):
  meesell-database-builder  → Alembic schema-split (product_images public→image); §0.10 RULED OPTION B — NO `products` read grant (shim-scoped repository)
...
| `app/repository.py` | NEW (from `modules/image/repository.py`) | 7 module-private methods bound to schema `image`. **§0.10 RULED OPTION B (founder 2026-06-13) applied here**: scope by `product_id` post-ownership-shim — the in-SQL `products` join is REMOVED. ZERO cross-schema SQL; the transitional `public.products` read grant is DISCARDED, not a fallback. | services-builder + database-builder |
...
| Postgres `image` schema + role | NEW (placeholder) | `CREATE SCHEMA image; GRANT ... TO image_user;` + `GRANT INSERT ON public.audit_events TO image_user` (worker audit). **§0.10 RULED OPTION B (founder 2026-06-13): NO `GRANT SELECT ON public.products TO image_user`** — tenancy resolves via the catalog ownership HTTP shim, so there is no cross-schema read to grant. |
...
- **Cross-schema-`products`-join (§0.10) resolved — RULED OPTION B (founder 2026-06-13)**: the HTTP-shim-scoped repository (scope by `product_id` post-ownership-shim, NO `products` join, NO read grant). The merge gate verifies ZERO cross-schema SQL read survives into the locked target AND that `image_user` holds no `products` grant.
...
- [ ] §0.10 RULED OPTION B (founder 2026-06-13): repository scopes by `product_id` post-ownership-shim, NO `products` join, NO silent cross-schema SQL, and `image_user` holds NO `products` read grant.
...
|---|---|---|---|---|
| RC1 | The cross-schema `product_images`↔`products` tenancy join (§0.10) breaks when `product_images` moves to schema `image` but `products` stays in `public`/catalog | **High** | High | **RULED OPTION B (founder 2026-06-13)**: scope repository by `product_id` AFTER the `assert_product_ownership` HTTP shim proves ownership (aligns §2.D "no cross-schema SQL"); the `products` join is REMOVED. NO read grant — Option (a) is DISCARDED, not a fallback. Was Risk #5-class; now RESOLVED at the design level. |
...
| v1.1 | 2026-06-13 | mesell-msc-tenancy-ruling-session-1 (backend lead) | **§0.10 cross-schema tenancy hazard RULED OPTION B (founder 2026-06-13).** Resolve image-service tenancy via the catalog `assert_product_ownership` HTTP shim, NOT a cross-schema DB read-grant. Repository scopes by `product_id` alone (post-shim); `products` join REMOVED; `image_user` granted NO `products` read access; the transitional read-grant (former Option A) DISCARDED, not a fallback. Aligns LOCKED §2.D ("no cross-schema reads — services talk via HTTP, never SQL"); zero MS-5 transitional debt. NEW precedent — MS-A recipe did not cover this (export owns its read path); recipe pattern added to backend-lead memory for MS-4/MS-5 inheritance. Flipped 7 surfaces: §0.10 hazard block, database-builder dispatch row, PHASE A dispatch line, repository.py file row, Postgres role-grant row, acceptance criterion, acceptance checklist item, RC1 risk row. |
