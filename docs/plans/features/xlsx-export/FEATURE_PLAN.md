...
```
## Trigger
This dispatch only fires if the monthly Meesho refresh (post
...
**Likelihood:** Medium. The 2026-06-04 snapshot is the baseline; Meesho has a history of periodic XLSX updates per `MEESHO_CATEGORY_INTELLIGENCE.md` §1, which our monthly refresh is sized to catch.
...
**Mitigation:**
- The conditional `scraper-maintainer` dispatch (§7.7) is the detection mechanism. Data lead runs the monthly diff and triggers if drift detected.
