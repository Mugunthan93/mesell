...
- docs/plans/features/feature_planning_master.md — the master tracker for all 9 feature planning sessions; READ it to see which features are LOCKED (you may depend on them) vs NOT STARTED / IN PROGRESS

## Step 0.5 — Update the master tracker (session start)
Before any other Step 1 work:
1. Read `docs/plans/features/feature_planning_master.md`
2. Find the row for `ai-autofill` (this feature)
3. Update its `Status` from `NOT STARTED` → `IN PROGRESS`
4. Update its `Last updated` column to today's date (use the `currentDate` from your context)
5. Append a one-line entry to the `## Recent updates log` at the top:
   `| {today's date} | ai-autofill | Planning session opened | mesell-ai-autofill-planning-session-1 |`
6. Save the file — DO NOT commit it yet

If your row is already `IN PROGRESS` (a prior session-was-interrupted scenario): proceed but flag this in the `## Decisions` section of FEATURE_PLAN.md so the founder knows.
...
## Step 8 — Update master tracker + commit + PR
1. Update `docs/plans/features/feature_planning_master.md`:
   - Find this feature's row (`ai-autofill`)
   - Update `Status` from `IN PROGRESS` → `PLAN READY`
   - Update `Last updated` to today's date
   - Update `Notes` with: outstanding founder decisions OR "ready for review" if none
   - Append a one-line entry to `## Recent updates log`:
     `| {today's date} | ai-autofill | FEATURE_PLAN.md ready for review | (PR pending) |`

2. Branch from develop:
   `git checkout -b feature/ai-autofill/planning`

3. Stage and commit:
   - `docs/plans/features/ai-autofill/FEATURE_PLAN.md` (the new file)
   - `docs/plans/features/feature_planning_master.md` (the tracker update)

   Commit message:
   `docs(plan): lock ai-autofill feature blueprint — agents, code surfaces, dispatch templates, review protocol`

4. Push to origin and open a PR to `develop` using the AI PR template format (AI is the most-involved lead — prompt-engineer owns the differentiating surface; eval gating with 0% invalid-enum is the primary acceptance signal).

5. After PR opens: come back to `docs/plans/features/feature_planning_master.md` one more time, update this feature's `Status` from `PLAN READY` → `IN REVIEW`, fill the `Notes` column with the PR URL, and amend the same commit (or push a follow-up commit with message `docs(plan): update tracker — ai-autofill IN REVIEW with PR link`).
