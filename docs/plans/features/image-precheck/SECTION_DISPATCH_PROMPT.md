...
| Worktree | `/tmp/mesell-wt/section-5-integration` |
| Branch | `feature/section-5/integration` |
| Boot session | `mesell-section-5-coordinator-session-1` (M=1) |
| Source templates | `docs/plans/repo_management/SECTION_DISPATCH_PROTOCOL.md` §3 (A), §4 (B), §5 (C) |

**Usage:** open a brand-new Claude Code session window → `cd /tmp/mesell-wt/section-5-integration` → `/rename mesell-section-5-coordinator-session-1` → paste **Template A** below (copy-paste ready, fully filled). Templates B and C are shells for the section-coordinator to fill LIVE after the check-in gate.
...
7. `docs/V1_FEATURE_SPEC.md` — Section 2 → Feature 5 (Image Pre-check) acceptance criteria; plus Section 3 (end-to-end user journey) + Section 6 (frontend routes) for your slice's surface.
...
## Template B — Tier-2 FRONTEND sub-session boot prompt (SHELL · section-coordinator fills LIVE)
...
- Rename this session now: `/rename mesell-section-5-frontend-session-1`
...
3. `docs/V1_FEATURE_SPEC.md` — Section 2 → Feature 5 (Image Pre-check); Section 3 (user journey) + Section 6 (Angular frontend routes) for your slice's UI surface.
...
## Template C — Tier-2 BACKEND sub-session boot prompt (SHELL · section-coordinator fills LIVE)
...
- Rename this session now: `/rename mesell-section-5-backend-session-1`
...
3. `docs/V1_FEATURE_SPEC.md` — Section 2 → Feature 5 (Image Pre-check); Section 4 (data model) + Section 5 (API endpoints) for your slice's contract surface.
