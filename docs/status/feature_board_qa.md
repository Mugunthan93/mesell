...
|---|---|---|---|---|---|---|
| qa-wave-1 | feature/qa-wave-1/e2e-codify | IN REVIEW | mesell-qa-wave-1-e2e-session-1 | 2026-06-22 | qa-coordinator gate (PR #385) | **CODIFIED + RUN — PR #385 → develop.** `meesell-e2e-test-writer` ran the two-phase mandate: explored LIVE on a port-isolated slot (shell :4210) and corrected `selector_registry.md` to the verified ground truth (the provisional registry was wrong in ~8 places, e.g. `nav-account`→`nav-profile`, `precheck-result-card`→`precheck-card`, `precheck-score`→none, `export-download-button`→`export-trigger`/`export-download`, `category-search`→`smart-picker-description`, `wizard-next`→`catalog-form-next`, `save-draft`→`catalog-save-status`, `catalog-list`/`catalog-card`→`dashboard-product-row`/`dashboard-empty-state`). All 8 flows fleshed out; `playwright test --workers=1` = **9 passed, 3 test.fixme** (stable ×3). GREEN: onboarding, catalog-creation, category-picker, plan-guard, **logout-guard (confirms the #381 logout fix E2E)**, google-signin(render), image-precheck(uploader renders), export(page renders). FIXME (reasons in `federation_quirks.md`): export-download (**product bug**: `export.component.ts` hardcodes `productId='current-product-id'`), image-precheck-card (local dev has no GCS → 502), google-signin-click (headless OAuth). Auth: worker-scoped authed-context fixture (single-use refresh rotation breaks shared storageState — see federation_quirks.md). `@playwright/test@1.52.0` added to devDeps. |
