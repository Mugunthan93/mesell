...
| qa-wave-1 | `feature/qa-wave-1/frontend` (gap-fill) | `IN REVIEW` | mesell-qa-wave-1-frontend-session-1 | 2026-06-22 | none | PR #383 open against develop. Fixed both mfe-pricing TS errors (:241 TS2352 + :266 TS2367) + net-new `auth.guard.spec.ts`. ng test frontend: 88/98 files pass, 1527 pass / 61 pre-existing fail. |
...
|---|---|---|---|---|
| frontend-coordinator | qa-wave-1 (frontend gap-fill) | **Fix the pre-existing mfe-pricing spec type errors that block the whole `ng test frontend` umbrella build.** | 2026-06-22 | RESOLVED — fixed in PR #383 by meesell-frontend-test-writer. |
