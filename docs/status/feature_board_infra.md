...
**Domain:** infra
**Last updated:** 2026-06-11 (ci-activation — PR #120 (develop→main, deploy fix) MERGED `75f30ea`; full main pipeline ran: **Gates 1-5 GREEN, Frontend 8/8 GREEN, Build GREEN for the FIRST time ever**; Deploy still RED — two more deploy-script bugs peeled: git-ref `origin/main`→FETCH_HEAD (already fixed on main via #123) and readyz `$(seq)` runner-expansion (fixed by my PR #127 on develop, awaiting a FRESH founder develop→main gate). Live dev unharmed: api 2/2 + worker 2/2 on def60521, /health 200.)
