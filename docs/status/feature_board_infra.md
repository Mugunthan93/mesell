...
**Domain:** infra
**Last updated:** 2026-06-12 (deploy-from-develop — **FIRST EVER FULLY HANDS-FREE DEPLOY. FOUNDER RULING 2026-06-12: dev deploys fire from develop, NOT main.** PR #137 squash `eb84779` → develop fired run **27388030304** = ALL GREEN end-to-end: Gates 1-5 ✅ · Frontend 8/8 ✅ · Build ✅ · **Deploy ✅**. Cluster now runs CI-built `api:eb84779...` (no longer by-hand def60521); `https://api.mesell.xyz/health` 200. ci-activation pipeline fully green — no remaining red. — ci-activation close-out recorded: **DONE — CI ACTIVE**: run 9 (`27366269839`, merge SHA `62713935`, PR #132) = FIRST FULLY GREEN end-to-end pipeline (5 gates + 8 frontend + Cloud Build + IAP deploy + external health 200); branch protection applied **develop-only** (13 required contexts + strict + 1 review; main intentionally left bare per founder ruling); `GEMINI_API_KEY_CI` still founder-pending (nightly ai_eval only, non-blocking); ci-activation row in Recently merged; readyz #127 + FETCH_HEAD #123 + rollout-settle preserved under the develop-only ref-guards.)
