...
**Domain:** infra
**Last updated:** 2026-06-11 (ci-activation re-fire #4 — Gates 1-5 ALL GREEN + first WIF auth green; Build RED on missing serviceAccountUser actAs binding for github-ci on compute SA — founder IAM-grant gate; run 27331720017)
...
|---|---|---|---|---|---|---|
| ci-activation | (no branch — CI-config / GitHub-settings ops) | BLOCKED | — | 2026-06-11 | founder — approve ONE additive IAM binding: `roles/iam.serviceAccountUser` for `meesell-github-ci` ON compute SA `888244156264-compute@...` (the Cloud Build runner). Build can't act-as the runner SA. | **Re-fire #4 — BIGGEST progress: GATES 1-5 ALL GREEN + FIRST-EVER WIF AUTH GREEN.** PR #112 (develop `c6f93e2`→main `a5cb4420`) MERGE-COMMIT `38587857e57d2632b2ed5d361e39e7f04636c2a1` (develop preserved). Run **27331720017** (main `3858785`): Gate1✅ Gate2✅ Gate3✅ **Gate4 integration✅ (saga CLOSED via #104/#107/#108/#110)** **Gate5 golden_roundtrip✅ FIRST-EVER (18p/821 deselected, real fixtures)** · **Build❌** · Deploy⏭(needs) · Frontend 8/8✅ · nightly+ai_eval⏭. WIF token-exchange WORKED (auth'd as meesell-github-ci; `vars.GCP_WIF_PROVIDER`==live provider EXACTLY, no var fix). Build RED at `gcloud builds submit`: `PERMISSION_DENIED: caller does not have permission to act as service account 117348555876726277669` = compute default SA `888244156264-compute@...` (this project's CB runner). github-ci has cloudbuild.builds.editor (submit) but NOT `iam.serviceAccountUser` on the runner SA (act-as). Old `meesell-ci` SA HAS that grant; `ci_identity` module never replicated it for the new SA. FIX = 1 additive `google_service_account_iam_member` (serviceAccountUser, github-ci → compute SA) in `infra/terraform/modules/ci_identity/main.tf` + apply (or imperative add-iam-policy-binding then codify). REPORTED — IAM mutation, NOT auto-applied (brief: report before mutating). Cloud Build exec + IAP deploy STILL UNPROVEN (build failed at submit; deploy skipped). Branch-protection DEFERRED until green-through-deploy. Cost ₹0. |
