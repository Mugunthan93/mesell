...
the LIVE seller flows — ``catalog.service.create_product`` (Feature 3) and the
``customer`` onboarding-complete path (ALL THREE recompute sites:
``upsert_profile``, ``set_active_categories``, ``set_compliance_extension``) —
and a Celery enqueue failure must NEVER break any of them.
...
* Make ``enqueue_category_scrape`` re-raise (remove its ``try/except``) →
  ...
  ``test_onboarding_complete_succeeds_when_monitor_enqueue_raises`` +
  ``test_compliance_step_succeeds_when_monitor_enqueue_raises`` go RED
  (the host flow propagates the broker error).
* Drop the false→true edge guard → ``test_repeated_patch_on_complete_profile``
  + ``test_compliance_step_repeated_on_complete_profile_does_not_reenqueue``
  go RED (re-enqueue storm).
"""
...
# ---------------------------------------------------------------------------
# Hook (b) — onboarding-complete via the FINAL COMPLIANCE STEP
# (set_compliance_extension — third recompute site; Director full-coverage
#  ruling: the onboarding trigger must fire on EVERY completion path.)
# ---------------------------------------------------------------------------
_SUPER_ID = "fashion"


def _compliance_profile_orm(*, onboarding_complete):
    """A duck-typed SellerProfileORM row with ``_SUPER_ID`` declared active."""
    row = _profile_orm(onboarding_complete=onboarding_complete)
    row.active_super_categories = [_SUPER_ID]
    return row


@pytest.fixture
def patched_compliance(monkeypatch):
    """Patch ``set_compliance_extension``'s collaborators (no live DB).

    The super_id gate passes because the profile declares ``_SUPER_ID``;
    the required-keys validation is forced to pass; the recompute is forced
    to True so the edge is driven solely by the ``existing`` flag.
    """
    user_id = uuid4()

    # super_id has a (non-None) spec so we reach the main recompute branch;
    # required-keys validation passes (no missing keys).
    monkeypatch.setattr(
        customer_service,
        "COMPLIANCE_EXTENSION_MAP",
        {_SUPER_ID: SimpleNamespace(required_keys=[])},
    )
    monkeypatch.setattr(
        customer_service, "_missing_required_keys", lambda spec, payload: []
    )
    monkeypatch.setattr(
        customer_service, "_recompute_onboarding_complete", lambda *a, **k: True
    )
    ...
    updated_row = _compliance_profile_orm(onboarding_complete=True)
    monkeypatch.setattr(
        customer_service.customer_repo,
...
@pytest.mark.asyncio
async def test_compliance_step_enqueues_per_distinct_leaf_on_edge(
    patched_compliance, monkeypatch
):
    """Completing onboarding via the final compliance step enqueues per leaf."""
    ctx = patched_compliance
    # Profile was previously INCOMPLETE → this call crosses the false→true edge.
    monkeypatch.setattr(
        customer_service.customer_repo,
        "find_by_user_id",
        AsyncMock(return_value=_compliance_profile_orm(onboarding_complete=False)),
    )

    db = AsyncMock()
    await customer_service.set_compliance_extension(
        user_id=ctx.user_id, super_id=_SUPER_ID, payload={}, db=db
    )
...
@pytest.mark.asyncio
async def test_compliance_step_repeated_on_complete_profile_does_not_reenqueue(
    patched_compliance, monkeypatch
):
    """A re-PATCH of an ALREADY-complete profile via compliance must NOT re-enqueue."""
    ctx = patched_compliance
    # Profile was ALREADY complete → no false→true edge this call.
    monkeypatch.setattr(
        customer_service.customer_repo,
        "find_by_user_id",
        AsyncMock(return_value=_compliance_profile_orm(onboarding_complete=True)),
    )
...
@pytest.mark.asyncio
async def test_compliance_step_succeeds_when_monitor_enqueue_raises(
    patched_compliance, monkeypatch
):
    """A broker outage on enqueue MUST NOT break the compliance-step flow."""
    ctx = patched_compliance
    monkeypatch.setattr(
        customer_service.customer_repo,
        "find_by_user_id",
