...
async def test_wrong_otp_locks_after_three_attempts(client):
    ...
    for _ in range(3):
        r = await client.post("/api/v1/auth/verify-otp", json={"phone": phone, "otp": "0000"})
...
async def test_returning_user_is_deduped(client):
    phone = "+917777777777"
    await client.post("/api/v1/auth/send-otp", json={"phone": phone})
    a = await client.post("/api/v1/auth/verify-otp", json={"phone": phone, "otp": "1234"})
    await client.post("/api/v1/auth/send-otp", json={"phone": phone})
    b = await client.post("/api/v1/auth/verify-otp", json={"phone": phone, "otp": "1234"})
