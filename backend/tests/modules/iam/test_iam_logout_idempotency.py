...
async def test_logout_first_call_revokes_then_second_call_is_noop(
    ...
    await vk.set(f"otp:{phone}", payload, ex=300)

    verify = await iam_service.verify_otp_and_issue_tokens(
        phone=phone, otp=otp, client_ip="192.0.2.10", db=db, valkey=vk
    )

    refresh_token = verify.refresh_token
    allowlist_key = refresh_allowlist_key(refresh_token)
    assert await vk.get(allowlist_key) is not None

    # First logout — must revoke.
    first = await iam_service.revoke_refresh_token(refresh_token, vk)
    assert first.cookie_was_present is True
    assert first.user_id is not None
    assert await vk.get(allowlist_key) is None
    ...
    second = await iam_service.revoke_refresh_token(refresh_token, vk)
    ...
    third = await iam_service.revoke_refresh_token(None, vk)
...
async def test_logout_with_unknown_token_does_not_raise(valkey):
    ...
    result = await iam_service.revoke_refresh_token(bogus, vk)
