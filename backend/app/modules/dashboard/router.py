"""``dashboard`` router — 1 endpoint handler per §13.B (LOCKED 2026-06-05;
...
``POST /api/v1/products`` from §10 catalog. FastAPI registers them as
two distinct ``APIRoute`` objects under one path key. The boot-integration
test's ``_route_map`` helper deduplicates by path, so ``len(route_map)``
stays at 27 distinct paths even though the raw ``APIRoute`` count rises
from 26 to 27 (post-§13 mount adds exactly 1 new ``APIRoute``).
"""
