...
async def _load_super_id_set(db: AsyncSession) -> list[str]:
    """SELECT DISTINCT super_id FROM categories — the JSON-serialisable
    list form used by the cache.

    Returned as a sorted list (deterministic for cache + tests).  The
    caller wraps the list in a set for O(1) membership checks.
    """
    stmt = select(CategoryORM.super_id).distinct().order_by(CategoryORM.super_id)
