"""svc-image async Valkey 8 client factories — DB-scoped, lazy-singleton.

Vendored from the monolith ``app.shared.valkey`` (BACKEND_ARCHITECTURE.md
§5.C), TRIMMED to the factories svc-image actually consumes:

* :func:`get_valkey_otp` (DB 0) — the per-IP / per-route rate-limit sliding
  windows (vendored rate_limit_mw) AND the SHARED ai_ops budget brake
  (``ai:cost:daily:{date}`` / ``ai:cost:pending:{date}`` /
  ``ai:budget:reservation:{id}`` — UN-prefixed, global ₹500/day cap per
  §0.5/D6; do NOT namespace these keys).
...
is a category/catalog concern, not an image one — also not vendored.
