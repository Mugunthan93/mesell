...
EXTRACTION NOTE (MS Sub-Plan C — spec §1 B1 / §16.G ABSOLUTE CONTRACT)
=====================================================================
This module is vendored from the monolith ``app.modules.image.service`` with
the pipeline / business logic preserved BYTE-FOR-BYTE.  The ONLY changes are
import lines:

* The cross-module import (monolith service.py:53
  ``from app.modules.catalog import service as catalog_service``) becomes
  ``from app.core.extracted_clients import catalog_client as catalog_service``
  — re-exporting the SAME ``catalog_service`` symbol name so the 2 call sites
  (``assert_product_ownership`` at :162 + :248) are UNCHANGED byte-for-byte.
* Intra-module imports are rewritten to the flat svc-image tree
  (``app.modules.image.{repository,domain,exceptions,schemas,tasks}`` →
  ``app.{repository,domain,exceptions,schemas,tasks}``).
* ``from app.adapters import gcs as gcs_adapter`` + ``from app.shared.config
  import settings`` are at the same flat paths.
...
from app.core.extracted_clients import catalog_client as catalog_service
from app import repository as image_repo
...
async def upload_image(
    ...
    *,
    ...
    from app.tasks import image_precheck_task
