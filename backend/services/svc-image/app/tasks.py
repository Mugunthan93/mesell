...
=====================================================================
Vendored from the monolith ``app.modules.image.tasks``.  The ONLY changes
are import lines:

* The lazy intra-module imports in ``_run_precheck_pipeline``
  (``from app.modules.image import repository as image_repo`` +
  ``from app.modules.image import service as image_service``) become
  ``from app import repository as image_repo`` +
  ``from app import service as image_service``.
* ``from app.ai_ops import budget_cap, client as ai_client`` (lazy, inside
  ``_check_watermark``) points at the VENDORED ``app.ai_ops`` package.
* ``app.adapters`` / ``app.shared.config`` / ``app.shared.database`` /
  ``app.shared.models.audit_event`` are at the same flat paths.
