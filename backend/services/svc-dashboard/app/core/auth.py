...
TRIMMED to the access-token VERIFICATION surface svc-dashboard needs (A2 / D7
— each service verifies the user JWT LOCALLY via the shared ``JWT_SECRET``).

The monolith's refresh-token machinery (opaque-token generation, HMAC-with-
pepper allowlist, the atomic rotation Lua script) is NOT vendored —
svc-dashboard ISSUES no tokens (it has no ``/auth/*`` routes) and reads no
``REFRESH_TOKEN_PEPPER`` (absent from the trimmed Settings).  It only verifies
the access JWT presented on the 1 dashboard route and proves the ``sub``
resolves to a live ``public.users`` row.
