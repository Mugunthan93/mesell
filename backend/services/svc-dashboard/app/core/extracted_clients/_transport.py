"""Shared HTTP transport for the 2 extracted_clients shims (spec §5.E).
...
----------------------------
The dashboard ``service.py`` call sites are byte-for-byte preserved and do NOT
...
(API path).  dashboard runs NO Celery worker, so there is no worker context
path — but ``set_worker_context`` is retained for transport-contract parity
with the pilot (svc-export) and is harmless.
"""
...
_bearer_token: ContextVar[str | None] = ContextVar("svc_dashboard_bearer_token", default=None)
_request_id: ContextVar[str | None] = ContextVar("svc_dashboard_request_id", default=None)
...
def set_worker_context(*, request_id: str | None) -> None:
    ...
    trust for the ``/internal/*`` routes).  Retained for transport-contract
    parity; svc-dashboard runs no Celery worker so it has no call site.
    """
...
async def request_json(
    ...
    *,
    ...
    unexpected 5xx surfaces as a failure).
    """
