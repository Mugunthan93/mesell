...
§11) with the autosave-coalescing branch retained (harmless for dashboard —
dashboard paths never match the autosave regex, so it is a structural copy).

Writes a row IFF response is 2xx AND ``request.state.user_id`` is set AND the
method is a write method.  dashboard's mounted route is a read-only ``GET`` —
the write-method gate means this middleware NO-OPs for dashboard in practice
(§13.B).  PII is scrubbed (phone hashed, FSSAI/GST stripped).  Drop-on-failure:
audit failure NEVER blocks the response.

NOTE on cross-schema write: ``AuditEvent`` is the vendored model bound to
``public`` (see ``app.shared.models.audit_event``), so any middleware INSERT
would land in ``public.audit_events`` even though svc-dashboard owns NO schema.
"""
