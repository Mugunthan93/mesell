"""svc-dashboard async Valkey 8 client factories — DB-scoped, lazy-singleton.

Vendored from the monolith ``app.shared.valkey`` (BACKEND_ARCHITECTURE.md
§5.C), TRIMMED to the factories svc-dashboard actually consumes:

* :func:`get_valkey_otp` (DB 0) — the per-IP / per-route rate-limit sliding
  windows + the audit-coalesce markers.

dashboard runs NO Celery worker, so the broker (DB 1) and result backend
(DB 2) factories from the monolith are NOT vendored.  The application
read-through cache (DB 3) is a category/catalog concern, not a dashboard one —
also not vendored.
