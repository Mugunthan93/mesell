"""Alembic environment for the svc-customer standalone migration chain.

Key differences from the monolith alembic/env.py:
  1. version_table_schema="customer" — the alembic_version tracking table
     lands inside the ``customer`` Postgres schema, not ``public``.  This
     prevents the svc-customer chain from colliding with the monolith chain's
     alembic_version row.
  2. No SQLAlchemy ORM metadata is wired (target_metadata = None).
     svc-customer migrations are hand-authored DDL; there is no ORM Base
     here to autogenerate from.
  3. DATABASE_URL is injected via the environment variable of the same name.
     It MUST point at the svc-customer Postgres role / schema (i.e. the
     ``customer`` schema must already exist, created by infra-builder's
...
# No ORM metadata for svc-customer migrations — all DDL is explicit.
...
def _get_database_url() -> str:
    ...
    if not url:
        raise RuntimeError(
            "DATABASE_URL environment variable is required to run svc-customer "
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        # version_table_schema: alembic_version row lives in the customer
        # schema so it does not collide with the monolith's public.alembic_version.
        version_table_schema=_CUSTOMER_SCHEMA,
    )
...
def do_run_migrations(connection: Connection) -> None:
    # Pre-create the customer schema before Alembic attempts to create the
    # version tracking table (customer.alembic_version).  Alembic's internal
    ...
    connection.execute(sa.text("CREATE SCHEMA IF NOT EXISTS customer"))
    connection.commit()
    ...
    context.configure(
        ...
        # version_table_schema: alembic_version row lives in the customer
        # schema so it does not collide with the monolith's public.alembic_version.
        version_table_schema=_CUSTOMER_SCHEMA,  # <-- THE KEY SETTING
