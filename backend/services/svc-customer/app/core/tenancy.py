...
dashboard owns NO repository and runs NO direct query (§13.D); the
``scope_to_user`` invariant for dashboard's data is enforced UPSTREAM at the
catalog + customer repositories (§10.D / §8.D), reached via the HTTP shims.
These helpers are vendored for chain-shape parity + the ``TenantViolationError``
the vendored core layer may raise; they have no in-process call site in
dashboard.
"""
