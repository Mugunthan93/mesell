...
:mod:`app.core.extracted_clients._transport` so the 2 HTTP shims (catalog +
customer) can forward the caller's user JWT (``Authorization`` bearer) +
correlation ID (``X-Request-ID``) to the monolith ``/internal/*`` routes
WITHOUT the byte-for-byte-preserved ``service.py`` call sites having to thread
them through.
...
This is svc-dashboard-specific (the monolith has no such middleware — its
dashboard module called the callees in-process with no header forwarding
needed).  It is NOT part of the §4.H 6-middleware chain count; it is an
extraction-support layer registered inside that chain.
"""
