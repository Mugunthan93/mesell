"""Alembic environment for the svc-export standalone migration chain.

Key differences from the monolith alembic/env.py:
  1. version_table_schema="export" — the alembic_version tracking table
     lands inside the ``export`` Postgres schema, not ``public``.  This
     prevents the svc-export chain from colliding with the monolith chain's
     alembic_version row.
  2. No SQLAlchemy ORM metadata is wired (include_schemas=False, no
     target_metadata).  svc-export migrations are hand-authored DDL; there
     is no ORM Base here to autogenerate from.
  3. DATABASE_URL is injected via the environment variable of the same name.
     It MUST point at the svc-export Postgres role / schema (i.e. the
     ``export`` schema must already exist, created by infra-builder's
...
# No ORM metadata for svc-export migrations — all DDL is explicit.
...
def _get_database_url() -> str:
    ...
    if not url:
        raise RuntimeError(
            "DATABASE_URL environment variable is required to run svc-export "
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        # version_table_schema: alembic_version row lives in the export
        # schema so it does not collide with the monolith's public.alembic_version.
        version_table_schema=_EXPORT_SCHEMA,
    )
...
def do_run_migrations(connection: Connection) -> None:
    # Pre-create the export schema before Alembic attempts to create the
    # version tracking table (export.alembic_version).  Alembic's internal
    ...
    connection.execute(sa.text("CREATE SCHEMA IF NOT EXISTS export"))
    connection.commit()
    ...
    context.configure(
        ...
        # version_table_schema: alembic_version row lives in the export
        # schema so it does not collide with the monolith's public.alembic_version.
        version_table_schema=_EXPORT_SCHEMA,  # <-- THE KEY SETTING (spec §3.C)
