...
EXTRACTION NOTE (MS Sub-Plan A — spec §3.A / §16.G ABSOLUTE CONTRACT)
=====================================================================
This module is vendored from the monolith ``app.modules.export.service`` with
the pipeline logic preserved BYTE-FOR-BYTE.  The ONLY changes are import lines:

* The 4 cross-module imports (monolith service.py:57-59 + :83) become
  ``from app.core.extracted_clients import <callee>_client as <callee>_service``
  — re-exporting the SAME ``<callee>_service`` symbol names so the 7 call sites
  (:174 / :177 / :185 / :314 / :452 / :503 / :659 in the monolith) are UNCHANGED.
* Intra-module imports are rewritten to the flat svc-export tree
  (``app.modules.export.{repository,domain,exceptions,schemas,tasks}`` →
  ``app.{repository,domain,exceptions,schemas,tasks}``;
  ``app.modules.category.exceptions`` → ``app.core.extracted_clients.category_client``).
* ZERO changes to any executable / non-import line.

Public surface (called from :mod:`.router`)
-------------------------------------------
* :func:`initiate_export` — POST /products/{id}/export-xlsx (6-step flow).
* :func:`get_export` — GET /exports/{id} (4-step polling flow).
...
from app import repository as export_repo
...
def _apply_strategy(
    ...
    from app.domain import (  # noqa: PLC0415
...
async def _translate_enums(
    ...
    *,
    ...
    """
    from app.core.extracted_clients.category_client import (
