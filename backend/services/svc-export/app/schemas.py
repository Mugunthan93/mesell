"""svc-export Pydantic v2 wire models per §14.G (LOCKED 2026-06-05).

Authoritative version — delivered by meesell-api-routes-builder (spec §3.B).
Wire shapes are byte-equivalent to the monolith ``app.modules.export.schemas``
so ``service.py`` is unaffected (§16.G zero-call-site change contract).

Wire shapes for the 2 endpoint surfaces:
  1. ``ExportRequest``                — POST /products/{product_id}/export-xlsx body.
  2. ``ExportInitiatedResponse``      — POST /products/{product_id}/export-xlsx 202 body.
  3. ``ExportResponse``               — GET /exports/{export_id} 200 body.
...
class ExportRequest(BaseModel):
    """POST /products/{product_id}/export-xlsx body.
...
class ExportInitiatedResponse(BaseModel):
    """POST /products/{product_id}/export-xlsx 202 response per §14.B.1."""
...
class ExportResponse(BaseModel):
    """GET /exports/{export_id} 200 response per §14.B.2.
    ...
    - ``completed_at``: always ``None`` in V1 (DDL has no column — set at
      service layer when status="ready"/"failed").
    - ``round_trip_validated``: ``True`` when ``status="ready"`` (MVP_ARCH §5.7
      invariant); ``None`` otherwise.  Derived from status at service layer.
    """
