"""``catalog`` public router — 6 endpoint handlers per §10.B (LOCKED 2026-06-05).

Extracted VERBATIM from ``backend/app/modules/catalog/router.py`` per §16.G
(ZERO call-site changes; only import paths rewired to the svc-catalog vendored
tree).
...
FEATURE_CATALOG_FORM_ENABLED mount guard (R9 / row-26)
-------------------------------------------------------
This router is mounted by ``app.main`` ONLY when
``settings.FEATURE_CATALOG_FORM_ENABLED`` is True (svc-catalog main.py
lines 118-128).  The internal router (``app.internal_router``) is NOT
gated — sibling services must always reach the ownership-check / export-
snapshot / list_products shims.
"""
...
from app.exceptions import DraftNotFoundError
