"""svc-catalog SQLAlchemy 2.0 async engine, session factory, FastAPI dependency.
...
(svc-catalog does a single-row read + single-row insert per request — see
``shared/config.py``).
...
helper for the Celery worker path.  svc-catalog runs NO Celery worker (it is a
...
class Base(DeclarativeBase):
    """Declarative base for svc-catalog's vendored ORM models."""
