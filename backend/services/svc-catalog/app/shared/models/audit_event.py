"""Audit event ORM model — vendored for svc-catalog, bound to ``public``.

Mirrors the monolith ``app.shared.models.audit_event.AuditEvent`` column
shape (MVP_ARCH §10 / §11.2) but drops the ``User`` relationship (svc-catalog
...
audit INSERT is a genuine CROSS-SCHEMA write (the I5 grant on the svc-catalog
