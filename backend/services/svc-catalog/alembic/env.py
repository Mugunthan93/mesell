"""Alembic environment for the svc-catalog standalone migration chain.

Key differences from the monolith alembic/env.py:
  1. version_table_schema="catalog" — the alembic_version tracking table
     lands inside the ``catalog`` Postgres schema, not ``public``.  This
     prevents the svc-catalog chain from colliding with the monolith chain's
     alembic_version row OR any other svc's alembic_version.
  2. No SQLAlchemy ORM metadata is wired (target_metadata = None).
     svc-catalog migrations are hand-authored DDL; there is no local ORM
     Base here to autogenerate from.
  3. DATABASE_URL is injected via the environment variable of the same name.
     It MUST point at a Postgres role that has ALTER TABLE rights on the 3
...
catalog.alembic_version — if the catalog schema does not exist yet, Postgres
...
# No ORM metadata for svc-catalog migrations — all DDL is explicit.
...
def _get_database_url() -> str:
    ...
    if not url:
        raise RuntimeError(
            "DATABASE_URL environment variable is required to run svc-catalog "
...
def do_run_migrations(connection: Connection) -> None:
    ...
    LOAD-BEARING ORDER:
    1. CREATE SCHEMA IF NOT EXISTS catalog + commit()  — MUST come before
       context.configure() so that _ensure_version_table() can write
       catalog.alembic_version.  (MS-A pilot gotcha; database-builder MEMORY.)
    2. context.configure(version_table_schema="catalog", ...)
    3. context.begin_transaction() + run_migrations()
    """
    # Step 1 — Pre-create the catalog schema BEFORE context.configure().
    ...
    connection.execute(sa.text(f"CREATE SCHEMA IF NOT EXISTS {_CATALOG_SCHEMA}"))
    connection.commit()
    ...
    context.configure(
        ...
        # inside catalog.alembic_version, isolated from all other chains.
        version_table_schema=_CATALOG_SCHEMA,  # <-- THE KEY SETTING (spec §3.C)
