"""svc-pricing async Valkey 8 client factories — DB-scoped, lazy-singleton.

Vendored from the monolith ``app.shared.valkey`` (BACKEND_ARCHITECTURE.md
§5.C), TRIMMED to the factory svc-pricing actually consumes:

* :func:`get_valkey_otp` (DB 0) — the per-IP / per-route rate-limit sliding
  windows + the audit-coalesce markers.

pricing runs NO Celery worker, so the broker (DB 1) and result backend (DB 2)
factories from the monolith are NOT vendored.  pricing is NOT a cache consumer
(spec §0.8) — the application read-through cache (DB 3) is also NOT vendored.
