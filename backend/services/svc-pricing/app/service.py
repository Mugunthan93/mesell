...
Per BACKEND_ARCHITECTURE.md §12.C (LOCKED 2026-06-05).  Vendored into
svc-pricing (spec §3.A) under the §16.G discipline: the ONLY changes vs the
monolith ``app/modules/pricing/service.py`` are (1) the import-line rewires
and (2) the §0.6 ProductORM elimination.  The 3 cross-module call sites
(:134, :165, :241 in the monolith) keep their TEXT byte-for-byte; the
``_compute_pnl`` / ``_q`` math + alert generation are verbatim.
...
------------------------------------------------------
In the monolith this module imported ``from app.modules.catalog import
service`` and ``from app.modules.category import service`` ONLY.  In
svc-pricing those are rewired to the HTTP shims
(``app.core.extracted_clients.catalog_client`` /
``category_client``), re-exported under the SAME ``catalog_service`` /
``category_service`` symbol names so the call sites are byte-for-byte
unchanged.  It NEVER imports another module's repository.  It NEVER imports
``app.adapters.gemini`` — pricing is deterministic math per §6A + §12.H
(no AI in V1).
...
§0.6 — the monolith read ``category_id`` via a shared-ORM
    ``db.get(ProductORM, product_id)`` (service.py:151-162).  That is an
    illegal cross-schema ORM read after extraction.  RESOLVED via Option B:
    the catalog ``/internal/products/{id}/ownership-check`` shim is widened to
    return ``category_id``; this service reads it over HTTP via
    ``catalog_service.get_category_id(...)`` and the ProductORM import +
    ``db.get`` are DELETED.
"""
...
from app import repository as pricing_repo
...
from app.domain import PnLBreakdown, PricingAlert, PricingCalc
from app.exceptions import CommissionMissingError
...
async def calculate(
    ...
    *,
    ...
    # Step 3 — obtain category_id (§0.6 resolution — Option B).
    # The monolith read ``product.category_id`` via a SHARED-ORM
    # ``db.get(ProductORM, product_id)`` (a read of the catalog-owned
    # ``products`` table).  After extraction that ORM read crosses the schema
    # boundary illegally (§2.D HTTP-only).  RESOLVED: the catalog
    # ``/internal/products/{id}/ownership-check`` shim is widened to return
    # ``category_id`` (Option B); we read it over HTTP here.  This call also
    # re-asserts ownership and surfaces a clean ProductNotFoundError on the
    # TOCTOU soft-delete race the monolith guarded at service.py:154-160.
...
def _compute_pnl(
    ...
    * ``profit_pct = profit / input_cost × 100``
    ...
    if denom <= Decimal("0"):
        ...
        from app.exceptions import InvalidPriceInputError  # noqa: PLC0415
