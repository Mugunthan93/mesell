...
TRIMMED to the access-token VERIFICATION surface svc-pricing needs (A2 / D7 —
...
svc-pricing ISSUES no tokens (it has no ``/auth/*`` routes) and reads no
``REFRESH_TOKEN_PEPPER`` (absent from the trimmed Settings).  It only verifies
the access JWT presented on the 1 pricing route and proves the ``sub``
