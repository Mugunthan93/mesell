"""Shared HTTP transport for the 2 extracted_clients shims (recipe §4).
...
----------------------------
The pricing ``service.py`` call sites are byte-for-byte preserved and do NOT
...
(API path).  pricing runs NO Celery worker, so there is no worker context
...
_bearer_token: ContextVar[str | None] = ContextVar("svc_pricing_bearer_token", default=None)
_request_id: ContextVar[str | None] = ContextVar("svc_pricing_request_id", default=None)
...
def set_worker_context(*, request_id: str | None) -> None:
    ...
    parity; svc-pricing runs no Celery worker so it has no call site.
    """
