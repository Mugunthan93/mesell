...
§11) with the autosave-coalescing branch retained (harmless for pricing —
pricing paths never match the autosave regex, so it is a structural copy).

Writes a row IFF response is 2xx AND ``request.state.user_id`` is set AND the
method is a write method.  pricing's mounted route is a write ``POST``
(``/products/{id}/price-calc`` with ``@audit_event("pricing.calculated")``) —
so for pricing this middleware ACTUALLY FIRES on the 2xx path and writes a row
(unlike dashboard's read-only GET, which NO-OPs the write-method gate).  PII is
scrubbed (phone hashed, FSSAI/GST stripped).  Drop-on-failure: audit failure
NEVER blocks the response.

NOTE on cross-schema write: ``AuditEvent`` is the vendored model bound to
``public`` (see ``app.shared.models.audit_event``).  pricing OWNS the
``pricing`` schema (``pricing_calcs``), so the audit INSERT into
``public.audit_events`` is a genuine CROSS-SCHEMA write (the I5 grant on the
svc-pricing role makes it land — recipe §5).
"""
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    async def _maybe_write(request: Request, response: Response) -> None:
        ...
        # Gate 2.5 — write-method only.  dashboard's mounted route is a GET,
        # so this gate is where audit NO-OPs for dashboard (§13.B).
