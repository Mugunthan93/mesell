...
svc-iam VENDOR NOTE (auth-builder, MS Sub-Plan G Phase B)
--------------------------------------------------------
This file is the byte-for-byte extraction of the monolith
``app/modules/iam/service.py`` into the standalone svc-iam tree.  The six
public-method PIPELINES are VERBATIM — the only change vs the monolith is the
import block, which adapts the monolith's nested ``app.modules.iam.*`` package
paths to svc-iam's FLAT layout (``app.repository`` / ``app.domain`` /
``app.exceptions``).  iam imports NO other domain module (it is all-✗ in the
call matrix — SUB_PLAN_0G §0.4), so there are ZERO call-site rewrites; the
``app.adapters.*``, ``app.core.auth``, ``app.core.metrics`` and ``app.shared.*``
imports resolve at the SAME paths in both trees.  The §7.I direct-ORM audit
SAVEPOINT path and the DPDP / webhook V1 gaps (SUB_PLAN_0G §0.8) are PRESERVED
VERBATIM — extraction preserves behaviour, it does not fix the gaps.
