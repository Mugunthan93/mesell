...
§11) with the autosave-coalescing branch retained (harmless for iam — iam's
``/auth/*`` + ``/webhooks/razorpay`` paths never match the catalog autosave
regex, so it is a structural copy).

Writes a row IFF response is 2xx AND ``request.state.user_id`` is set AND the
method is a write method.  iam's mounted routes are write ``POST``s
(``/auth/otp/verify`` / ``/auth/refresh`` / ``/auth/logout``) carrying
``@audit_event(...)`` — so for iam this middleware FIRES on the authenticated
2xx path and writes a row.  PII is scrubbed (phone hashed, FSSAI/GST stripped).
...
``public`` (see ``app.shared.models.audit_event``).  iam OWNS the ``iam``
schema (``users``), so the audit INSERT into ``public.audit_events`` is a
genuine CROSS-SCHEMA write (the ``GRANT INSERT ON public.audit_events TO
iam_user`` makes it land — recipe §5).
"""
