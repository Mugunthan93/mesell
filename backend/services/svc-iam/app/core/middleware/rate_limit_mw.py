...
* Per-route — only when the handler is decorated with :func:`rate_limit`.  iam's
  public auth routes carry per-IP windows (``otp_send`` 3/3600, ``otp_verify``
  10/3600, ``auth_refresh`` 60/3600) — applied on the route handlers in
  ``router.py`` (api-routes-builder's file); ``logout`` / ``me`` /
  ``webhooks/razorpay`` carry no per-route decorator (idempotent / per-IP-floor
  / HMAC-gated).
