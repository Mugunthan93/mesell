"""Valkey DB 3 read-through cache + ETag helper + pre-warm hook (svc-category).

Vendored from the monolith ``app.core.cache`` (BACKEND_ARCHITECTURE.md §4.D).
svc-category is the HEAVIEST read-through consumer (schema + field-enum +
category tree + super-category list, §6.7).

Key format (vendored + §2.E namespace)::

    meesell:v{cache_version}:category:{caller_key}

The ``category:`` segment is the §2.E per-service namespace prefix applied in
:func:`_versioned_key` (the SOLE DB-3 key-construction site).  It is scoped to
THIS cache factory only — the SHARED ai_ops budget keyspace (``ai:cost:*`` /
``ai:budget:*`` on DB 0, built by the vendored ``ai_ops.budget_cap`` /
``cost_tracker``) is UN-prefixed and GLOBAL (F3.c / R1).  ``cache_version``
bumps on the quarterly Meesho corpus refresh — invalidates all schema / enum /
category-tree keys atomically without ``FLUSHDB``.
...
Pre-warm (MVP_ARCH §6.7) — LIVE in svc-category
-----------------------------------------------
:func:`prewarm_top_categories` is wired from ``app/main.py`` startup; in
svc-category it is LIVE (not a stub) — it warms the full ``category_tree``
key + the top-N ``schema:{id}`` keys via the vendored ``app.service`` (the
flattened svc tree).
"""
...
# ── §2.E per-service namespace ─────────────────────────────────────────────
# Applied ONLY here, on DB-3 cache keys.  The SHARED ai_ops budget keyspace
# (DB 0, ``ai:cost:*`` / ``ai:budget:*``) is built by the vendored ai_ops
# modules and NEVER passes through this function, so it stays un-prefixed /
# global — the ₹500/day cap is SHARED (F3.c / R1).
_SERVICE_NAMESPACE = "category"


def _versioned_key(key: str, version: str | None) -> str:
    """Build the ``meesell:v{cv}:category:{key}`` key.

    The ``category:`` segment is the §2.E per-service namespace prefix — it
    applies ONLY to category's own DB-3 read-through cache keys, never to the
    shared DB-0 budget keyspace.
    """
    cv = version if version is not None else settings.CACHE_VERSION
    return f"meesell:{cv}:{_SERVICE_NAMESPACE}:{key}"
