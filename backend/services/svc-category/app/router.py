"""svc-category public router — 5 endpoint handlers per §9.B (LOCKED 2026-06-05).

VERBATIM port from ``backend/app/modules/category/router.py`` (monolith) with
only the import paths adjusted to the svc-category vendored package tree.
Logic, decorators, ETag/304 behaviour, flag guard, and docstrings are
BYTE-IDENTICAL to the monolith source (§16.G call-site preservation).
...
Import paths vs monolith
------------------------
Monolith:  ``from app.modules.category import service as category_service``
svc-category:  ``from app import service as category_service``
(The service is at the package root in the extracted tree.)

Monolith:  ``from app.modules.category.schemas import ...``
svc-category:  ``from app.schemas import ...``

All other imports (auth, cache, rate_limit, config, database) point at the
SAME vendored paths both trees share (``app.core.*``, ``app.shared.*``).
"""
