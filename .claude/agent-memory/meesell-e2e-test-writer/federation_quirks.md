...
## DEPLOYED stack (GitHub Pages + Fly.io) quirks — qa-pricing e2e lane (2026-07-06) [SCRIBED BY QA-COORD, isolated-writer scribe]
> The qa-pricing e2e writer explored against the LIVE deployed build (github.io Pages front +
> Fly.io backend) because the local 8GB box couldn't stand a full stack. Five deployed-only
> gotchas were found — they explain why the live pass/fail run is OWED and the specs were
> disclosed-as-not-run (list-clean + source-ground-truth fallback, per the e2e gate allowance).

- **(1) Deployed auth is RELOAD-FRAGILE (cross-site refresh cookie).** The refresh-token cookie is `Domain=.fly.dev` + `SameSite`-restricted, so it is NOT sent on a request originating from `github.io`. Consequence: any full-page `page.goto(...)` RELOADS the SPA, the APP_INITIALIZER refresh has no cookie → 401 → the deep-link lands on `/login`. WORKAROUND used live: navigate CLIENT-SIDE only (never `goto` a protected deep-link mid-session); for a deep link, push the URL via `history.pushState` + dispatch `popstate` so the Angular router handles it in-memory WITHOUT a reload. This is a DEPLOYED-topology artifact (cross-origin split), NOT the local FED-1 singleton logout.
- **(2) Deployed base-href is `/mesell/`.** GitHub Pages serves the app under `/mesell/`, so a leading-slash `page.goto('/catalogs/...')` 404s / mis-resolves. Prepend the base-href (or drive via the router/pushState) on the deployed target. Local dev is base-href `/` — so this is a deployed-only divergence; keep gotos config-relative.
- **(3) DEPLOYED BLOCKER — product creation is DOWN.** `POST /categories/suggest` returns EMPTY suggestions on the deployed backend AND the category browse tree is UNSEEDED → `createProductViaPicker()` cannot mint a real product on deployed → the pricing/catalog-delete flows that need a real product cannot run live there. A ~31-call `/suggest` RETRY-STORM was observed (empty responses trigger the client to keep retrying). This is why the pricing apply trio was live-verifiable (static controls render on load) but the FULL calc→apply flow + W3-E2-4 delete were only SOURCE-ground-truthed, not live-run. Un-block needs a seeded deployed DB or a local full stack.
- **(4) Deployed onboarding does NOT persist `onboarding_complete`.** After the onboarding PATCH, a re-login routes the user back to `/onboarding` (the flag never flipped on the deployed backend). Matches the local onboarding-persist history but confirmed again on deployed. Don't assume a one-time onboarding on the deployed target.
- **(5) OFFLINE Playwright harness recipe (deployed/air-gapped runs).** `pnpm add --offline @playwright/test@1.52.0` (matches the cached chromium-1169 → no browser download) + run with `PLAYWRIGHT_BROWSERS_PATH=$HOME/Library/Caches/ms-playwright`. `@playwright/test` is CommonJS → in an ad-hoc `.mjs` probe import as `import pw from '<abs>/@playwright/test/index.js'; const { chromium } = pw;`. Same recipe as the Wave-C harness note above; re-confirmed for the deployed exploration.
