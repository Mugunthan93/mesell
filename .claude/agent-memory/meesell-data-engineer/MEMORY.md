...
**Wave 1.5 (commission backfill) — BLOCKED, reusable finding:** the Meesho commission rate-card scrape HARD-STOPPED. Two simultaneous walls: (1) **Playwright MCP is not configured** in headless dispatch (only `pencil` MCP registered in claude_desktop_config.json — same finding recorded 2026-06-06); (2) **Akamai WAF 403s ALL server-side HTTP** to Meesho across 17 probed URLs (supplier/www/seller/help subdomains) — validates TLS fingerprint (JA3/JA4), so curl + UA-spoof can't bypass; public subdomains serve a JS-only 2,166-byte SPA shell; even `robots.txt` 403s. So Meesho cannot be scraped server-side; quarterly refresh needs a real browser + authenticated supplier session. `backend/app/data/category_commissions.json` is an empty placeholder w/ `_meta` hard-stop record. Path forward (awaiting founder): **Option A** = founder manually copies the ~30-50-row rate-card from the supplier panel → I structure it + author `scripts/seed_category_commissions.py` for idempotent backfill; **Option B** = add `@playwright/mcp` to Claude Desktop config + re-dispatch scraper-maintainer interactively with supplier OTP (correct quarterly-refresh path). Mapping caveat: Meesho publishes commission at category/sub-category granularity, not per-leaf — 5 overlapping super clusters (Personal Care/Home/Kids/Automotive/Office-Craft) need founder disambiguation when values arrive. Capture report: `docs/plans/architecture/CATEGORY_SEEDING_COMMISSION_CAPTURE.md`.
