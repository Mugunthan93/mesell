...
2026-06-22 — Dev-google bypass seam (#480): guard at top of `adapters/google.py::verify_id_token` — `_resolve_dev_bypass_claims` checks `APP_ENV=='production'` FIRST (returns None → leaked prod token inert), then config+flag, then constant-time `secrets.compare_digest` of sentinel `dev-google:{sub}:{email}` → synthetic verified claims; else None → real verify unchanged. Mutation-proven prod-disable. Twin `services/svc-iam/.../google.py` not mirrored (monolith-only, V1.5).
