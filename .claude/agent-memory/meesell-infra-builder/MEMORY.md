...
## e2e-port-reconcile — auth E2E dev-stack two-fix (2026-06-22, session mesell-e2e-port-reconcile-infra-session-1)

**INFRA-1 diagnosis = COMMITTED-FILE bug, but NOT where the brief guessed.** The brief described a manifest(declaration-order) vs running(alphabetical) split. By the time I looked, **PR #412 (Option 1, founder-chosen; merge `437c68d`) had ALREADY unified the regime to SORTED** (mfe-auth=4201 … mfe-pricing=4207) across federation.manifest.json == angular.json serve == ci.yml == serve-static.mjs == tools/meesell_env.py slot-0. The ONLY remaining committed divergence was `frontend/e2e/playwright.config.ts` REMOTE_PORTS + header comment, still on the OLD pre-#412 order (added on the parallel qa-wave-infra branch that never saw #412). That mismatch IS the "/login → wrong remote → Unknown exposed module → D12" symptom. Fix = PR #447 (1 file, port literals + comment, +21/-15, no spec/app change), REMOTE_PORTS now == committed manifest. **CANONICAL = SORTED/alphabetical (#412 Option 1).** meesell_env.py `ports_for_slot` uses `enumerate(sorted(discover_mfes()))` → slot-0 IS the sorted canonical ref; angular.json serve ports were flipped to sorted by #412 too.

**LESSON #1 — ALWAYS `git fetch origin develop` + read the BOARD before diagnosing a "stale committed file."** My local develop `e82d52f` was 2 days behind `9d5f5b4`; reading the stale tree made me initially conclude angular.json was the OLD order and meesell_env was the bug — the OPPOSITE of reality. The board's `federation-manifest-reconcile` row (PR #412) recorded the founder's Option-1 ruling and the whole reconcile. Reading my own board first would have saved a wrong first hypothesis. The board IS the founder's single query surface AND mine.

**LESSON #2 — the qa-e2e lane authored a port-mismatch WORKAROUND (`applyManifestPortFix`, env `MEESELL_FIX_MANIFEST_PORTS=1`, in `frontend/e2e/fixtures/auth.ts`, doc'd in e2e-test-writer/federation_quirks.md).** It rewrites the served manifest FROM `REMOTE_PORTS`, so it is only correct if playwright.config.ts is canonical — which is why PR #447 (fixing REMOTE_PORTS) is the real fix; with the canonical regime + a fresh build the workaround is a no-op (served manifest already == running ports).

**INFRA-2 — slot-0 baseline rebuild GOTCHA: the master tree (slot-0) source can be contaminated by a live sibling session.** The master tree's `apps/mfe-onboarding/src/app/onboarding.component.ts` working copy was 369→259 lines with the 7-field Legal-Metrology form RIPPED OUT (a sibling FE session's WIP), so `meesell_env.py baseline refresh` / any master-tree build would re-bake the sibling's WIP, NOT develop tip. Master tree had **75 uncommitted files** across concurrent sessions → never git it, never build from it when verifying-vs-develop. FIX PATTERN: build the affected apps from a CLEAN origin/develop-tip worktree (`pnpm install --frozen-lockfile` reuses the store, ~4s), then re-serve them on the slot-0 ports by killing ONLY the target serve.js pids (verify `ps -o command= | grep serve.js` before each kill) and relaunching `node frontend/tools/boot-smoke/serve.js <clean-wt-dist>/<app>/browser <port> [backend-url-for-shell]`. NEVER touch :8000 (uvicorn) or the other remotes. Shell needs the 3rd arg `http://127.0.0.1:8000` for the /api reverse-proxy (dev-proxy PR #367 fix).

**VERIFY THE SERVED BYTES, not the source.** `grep` the served chunk (curl :4206/OnboardingComponent-<hash>.js) for the field string literals (`manufacturer_pincode` etc. survive minification as formControlName keys) AND `diff -q` it against the clean-build chunk (byte-identical). Then browser-confirm with agent-browser: `eval` for `[data-testid=login-phone-input]` presence + a `federationFailure` regex over innerHTML (Unknown exposed module|RemoteFailure|Unable to resolve specifier). /onboarding redirect→/login under agent-browser is the authGuard doing its job (federationFailure=false), NOT a stale bundle — a full authed render needs the OTP flow.

**esbuild --service deadlock is real + intermittent here** (own memory: "baseline up KNOWN ISSUE"). Symptom: ng build + esbuild both at 0.0% CPU for minutes, partial hashed chunk written, no "Output location" line. Caught it via two CPU samples 10s apart. FIX: `kill -TERM` the ng pid + `pkill -f esbuild`, then retry — cleared on the next attempt. NOTE: 0.0% CPU AFTER a finished build is also normal (esbuild service idles post-build) — distinguish by whether the log has "Output location:" (done) vs not (deadlocked).

Final running slot-0 stack (left up for the e2e lane): :4200 shell(+proxy→8000) / :4201 mfe-auth / :4206 mfe-onboarding from clean wt dist; :4202-4205,4207 from master dist (runtime-identical to develop, only .spec deltas); :8000 backend untouched. PR #447 → develop (founder gate). ₹0, dev-only.

---

## Agent-memory permission remediation — 2026-06-22 (session `mesell-agent-memory-permfix-session-1`)

**Symptom (recurring):** isolated `meesell-*` agents repeatedly CANNOT write their own `MEMORY.md` (blocked scraper-maintainer, database-builder, data-engineer, backend-coordinator this session; historically forced git-plumbing scribes PRs #387/#388). Root cause = OWNERSHIP, not the editor.

**Diagnosis:** 164 paths under `.claude/agent-memory/` were `root:staff` (159 files `0644` = root-write-only; **5 dirs `0755`** = e2e-test-writer, backend-test-writer, qa-coordinator, frontend-test-writer, section-coordinator — a root-owned DIR blocks the user from CREATING new files in it, the worst case). Spread across 23 agent dirs. The agent-memory dirs' PARENT (`.claude/agent-memory/` itself) was already user-owned — only contents were root.

**RECURRENCE CAUSE — CONFIRMED & ONGOING:** the Bash/runtime in some sessions executes under sudo as ROOT — `EUID=0 USER=root SUDO_USER=mugunthansrinivasan`. Any memory write in such a session lands `root:staff 0644`. mtimes proved it's LIVE (backend-coordinator + director memory written as root at 16:53/16:58 the same day). So `whoami` here returns `root` — **NEVER chown to `$(whoami)`** (would entrench the bug); the intended owner is always `mugunthansrinivasan:staff` (the SUDO_USER / every healthy dir's owner).
**Prevention suggestion for founder/Director:** stop running the agent session as root/sudo (run Claude as `mugunthansrinivasan`), OR add a session-start/PreToolUse hook that `chown`s `.claude/agent-memory` back to `$SUDO_USER` (or runs the memory-write step de-escalated via `sudo -u "$SUDO_USER"`). Until then this chown must be re-run whenever a root session writes memory.

**Fix applied (surgical, agent-memory ONLY):**
```
chown -R mugunthansrinivasan:staff .claude/agent-memory/
chmod -R u+rwX .claude/agent-memory/      # dirs traversable+writable, files rw; capital X avoids spurious +x on files; NOT world-writable
```
Verified: post-fix `find .claude/agent-memory -not -user mugunthansrinivasan` = 0; all 159 pre-chown SHA sums re-matched (content untouched); real test write+delete of a scratch file AND append+byte-revert to qa-coordinator/MEMORY.md (a formerly root-owned dir) both succeeded as the user.

**Left UNTOUCHED (out of scope — reported for founder decision, NOT blocking memory writes):** `.claude/hooks`, `.claude/skills`, `.claude/settings.json`, `.claude/settings.local.json`, `.claude/scheduled_tasks.lock` (all root-owned config/hooks), and the thousands of root-owned paths under `.claude/worktrees/` (ephemeral agent git checkouts — same root-execution cause; prune/own those via the worktree mechanism, not a blanket chown).

**GOTCHA for myself:** when I (infra-builder) run as root and need to write a user-owned file, write it via `sudo -u mugunthansrinivasan tee -a <file>` (append) — a plain Edit/Write tool call from a root session would re-create the very root-ownership bug. This memory entry itself was appended via `sudo -u`.

---

## Post-QA-wave housekeeping — superseded-PR close + dirty-master-tree reconcile (2026-06-22, session `mesell-infra-housekeeping-scribe-infra-session-1`; PR #473)

**TASK 1 — closing "superseded board-scribe" PRs without REGRESSING a board. METHOD that protects me:** a board-scribe PR records an INTERMEDIATE gate state (e.g. "Wave A RE-GATE → merged to integration", or a "Wave B REJECT" row). It is safe to CLOSE (never MERGE) only if (a) the PR is board/docs-only AND (b) the FINAL outcome it described already landed on develop via the real wave merge. VERIFY (b) concretely: find the wave merge commit (`gh pr view <wave-PR> --json mergeCommit,state`; here qa-catalog #470 = `494c783`, in develop tip `4138ead` ancestry via `git merge-base --is-ancestor`). **CRITICAL CATCH that saved me from data-loss:** two of the three candidates (#454 Wave B REJECT, #466 Wave C scribe) ALSO bundled `.claude/agent-memory/meesell-qa-coordinator/` + `meesell-e2e-test-writer/` MEMORY appends that were GENUINELY NOT on develop (verified: grep the develop copy of each memory file for a signature phrase from the PR's added lines → absent). The wave merge (#470) was CODE/TEST ONLY — it did NOT carry those memory learnings. So I CLOSED only the pure-board one (#443) and LEFT-OPEN+REPORTED #454/#466. RULE: a board-scribe PR is "pure board" only if `gh pr view --json files` lists ONLY `docs/status/feature_board_*.md`; if it also touches `.claude/agent-memory/**`, treat the memory as potentially un-landed and do NOT close until you've proven each memory append is already on develop. Closing a PR discards its un-merged blobs — non-recoverable except from the branch ref, which a board-cleanup might also prune.

**TASK 2 — reconciling a STALE master tree that diverged + accumulated cross-session dirt. The disposition forensics (do this per-file, don't eyeball):**
- The master tree was a stale snapshot (HEAD 48 behind origin/develop) → its working-tree edits are RELATIVE TO THE OLD BASE. So `git diff origin/develop -- <file>` for a CODE/spec/test file showing **`0 added / N removed`** (or few-added/many-removed) = the worktree is the OLDER version and develop ADDED the N lines = STALE REVERT, safe to discard. Confirm a sample by `git log -1 origin/develop -- <file>` (the develop commit that touched it postdates the merge-base) and `git cat-file -e <merge-base>:<file>` (existed on base → develop evolved it, worktree reverted it). This pattern held for ALL backend/**, frontend/** code+spec+test files + the `dead_route_guard.mjs` delete + `pnpm-lock`/`angular.json`/`federation.manifest`/`ci.yml`.
- Files with NET ADDITIONS the worktree has over develop (`+N/0` or `+N/M`) = candidate un-landed narrative — but ONLY for append-only STATUS/board/MEMORY files, and a stale-base can ALSO produce spurious `+` lines, so VERIFY each section header is genuinely absent on develop (`comm -23 <(sort worktree) <(sort develop)` to get the truly-unique lines; then grep develop for each unique section header). My OWN infra MEMORY had exactly 2 such sections (e2e-port-reconcile + agent-memory-permfix); STATUS_INFRA had exactly 1 (the e2e-port-reconcile UPDATE block — develop only REFERENCED it from a later block, never had the block itself).
- **SOLE-WRITER BOUNDARY is the load-bearing constraint here:** a dirty master tree is a SHARED surface — it had 60 dirty files spanning ALL 6 domains' STATUS/board/memory + 16 agent-memory dirs + 2 other-lead commits. I may ONLY land MY infra sole-writer files (`STATUS_INFRA.md`, `feature_board_infra.md`, `meesell-infra-builder/**`). Other leads' stranded STATUS_AI/BACKEND/DATA/FRONTEND + their boards + memory dirs — even when genuinely un-landed — are NOT mine to land. I DISCARD them from the tree (via FF) but PRESERVE them zero-loss + REPORT, so the owning lead re-scribes. Never bundle another lead's file into my consolidated docs PR.

**ZERO-LOSS FF pattern for a DIVERGED + dirty master tree (the 2 local commits made `merge --ff-only` impossible):**
1. `tar -czf /tmp/<name>.tgz $(git status --porcelain | sed 's/^...//')` — snapshot the entire dirty surface (tracked-dirty + untracked) to a tarball.
2. `git branch backup/<name>-<date> develop` — a recoverable ref for any local commits ahead of origin (here 2 data-engineer STATUS_DATA scribes).
3. `git stash push -u -m "<why>"` — zero-loss stash of the dirty tree (tracked + untracked). The stash is the live recovery handle; do NOT pop it (it would re-dirty the tree with the stale reverts).
4. `git merge --ff-only origin/develop` (will `fatal: Not possible to fast-forward` if diverged) → fall back to `git reset --hard origin/develop` (the local commits survive in the backup branch).
5. Verify `git status` clean + `git rev-parse develop == origin/develop`.
Three independent safety nets (stash + backup branch + tar) = any "ambiguous, could-be-real-work" file the brief warns about is recoverable; I never have to GUESS-discard.

**Master-tree git guard nuance:** even from a LEGITIMATE worktree (`/private/tmp/mesell-wt/infra-scribe`, gitdir `…/.git/worktrees/infra-scribe`), the PreToolUse `guard-master-tree-git` hook tripped on `git commit`/`git add` (it keys on the shared `.git` path, not the worktree toplevel). STAGING from inside the worktree dir worked, but COMMIT needed the sanctioned `MESELL_ALLOW_MASTER_GIT=1` prefix — used deliberately for this housekeeping recovery (NOT a master-tree commit; the branch is a real worktree branch off origin/develop). `--no-verify` alone does NOT bypass the Bash PreToolUse guard (that's git's own hooks); the env override is the documented escape.
