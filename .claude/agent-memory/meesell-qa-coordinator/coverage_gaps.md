...
## qa-pricing e2e lane EXECUTED — gate PASS on `1794da3` (2026-07-06, mesell-qa-wave-pricing-coord-session-2)
The e2e-test-writer executed SPEC A + SPEC B group R (direct-to-develop pipeline `1794da3`). Post-hoc merge-gate = **PASS**. Gap ledger updated:
- **CLOSED — W3-E2-4 (catalog-edit-delete):** de-fixme'd → real inline-confirm delete round-trip (create→list→`catalog-delete-btn`→`catalog-delete-confirm`→`rowFor(id).toHaveCount(0)`). Consumes the #425 testids. Inter-lead request to frontend (catalog-list delete + testids) → **RESOLVED**.
- **CLOSED — W3-E2-6 (price-apply-export):** de-fixme'd → real calc→apply→export-page-reachable chain. Consumes the #439/SPEC-C apply trio. Inter-lead request to frontend (mfe-pricing apply control + testids) → **RESOLVED**.
- **NEW COVERAGE — PQE-E2E-04 + 04b:** apply-price → export-page navigation (URL + Generate visible), and the apply-error path (route-stub `**/apply-price`→500 set AFTER a real calc → `pricing-apply-error` visible, stays on `/pricing`). No green-wash.
- **STILL OPEN — CAT-E2E-03/07 (frontend testid + live-verify):** kept `test.fixme`. The role selectors were source-confirmed @ `c5529f6` but NOT live-verifiable (deployed suggest empty → fallback UI never renders; agent-browser can't route-stub `fallback_offered`) — two-phase mandate forbids un-fixme from source alone. **Testid memo now FILED → frontend-coordinator: `smart-picker-browse-fallback` / `smart-picker-empty` / `smart-picker-empty-browse`** (new inter-lead row on the board). Un-fixme when the testids land OR the roles are agent-browser live-verified on a working stack.
- **STILL OPEN (unchanged, SPEC B):** CAT-E2E-05/06 (category-schema seed + Gemini canned-JSON stub), PQE-E2E-05 + image-precheck (storage → **fake-gcs-server**, NOT MinIO — the adapter is `google.cloud.storage`), W3-E2-2 live-preview (product gap #278 — OQ-2 delete-or-retain, founder). All gated on the not-yet-existing E2E CI job (infra) + the storage/seed enablement.
- **Suite state:** `test.fixme` 9 → **7**; `playwright test --list` CLEAN 31/16. **Live pass/fail run still OWED** on a working full stack (deployed product-creation down + local 8GB swap-blocked) — the highest-value standing e2e debt across all waves. Accepted under the standing e2e gate allowance (list-clean + source-ground-truth + deployed live-verify of static controls), fully disclosed.
