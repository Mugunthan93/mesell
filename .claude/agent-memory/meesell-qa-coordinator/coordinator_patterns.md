...
## From Wave 3 catalog `integration → develop` gate (PR #460, 2026-06-22) — FOUNDER-DIRECTED merge

- **The conftest `_provision_test_schema` autouse fixture is the gate's friend for a DB-vs-branch alembic mismatch** → backend lane / gate environment → when the local `meesell_test` is stamped at a revision NOT in the PR branch's migration history (here `480c10b0219f`, while the branch head is `e9415bdcae20`), DO NOT manually stamp/downgrade. Just run pytest with `TEST_DATABASE_URL` set — the session-scoped autouse fixture DROPS the public schema + runs `alembic upgrade head` against the branch's OWN chain before any integration test touches the DB. This is exactly how CI Gate-4 provisions. Verify afterward (`SELECT version_num FROM alembic_version` == branch head) to prove the integration tests ran against the right schema and weren't silently `pytest.skip`'d on a schema mismatch.
- **Pure-function-mirror / imported-model-fn FE specs are runnable via STANDALONE vitest — the native-federation builder can't scope `--include`** → frontend lane / gate mechanics → `@angular/build:unit-test` with a `@angular-architects/native-federation:build` buildTarget returns "No tests found" for EVERY narrow `--include` path/glob (exact file, project-root-relative `../../apps/...`, workspace-root-relative, directory glob — all fail); only the broad committed `**/*.spec.ts` glob walks them, which is the full ~1700-test RAM-heavy run. BUT specs that import only TYPES + real model pure-functions (zero Angular/TestBed/PrimeNG runtime imports — grep the import block to confirm) run cleanly under `node_modules/.bin/vitest run <spec1> <spec2> --globals` in seconds. This is the RAM-safe scoped-FE-gate path; reserve the full `ng test frontend` for specs that genuinely need the Angular Vite plugin. (Always grep imports first: a TestBed/component import means you CANNOT shortcut to standalone vitest.)
- **A 72-commit develop advance past the rebase point is NOT a merge blocker when the diffs are disjoint** → all QA waves / founder-gate → trust `mergeStateStatus: CLEAN` + `mergeable: MERGEABLE`. Here develop moved from the rebase base `0efd3ef` by 3 commits (#480/#481/#482, all non-test source/docs) while the integration branch added 3 QA-lane commits (test files only) → disjoint file sets → the merge-commit reconciled with zero conflicts. Compute the TRUE lane delta with `git diff origin/develop...integration` (3-dot) — it correctly excludes develop's advances and shows only the lane's tests-only contribution. Don't be alarmed by "develop advanced N commits"; be alarmed only by a DIRTY/CONFLICTING state or a non-test file in the 3-dot diff.
