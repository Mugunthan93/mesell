...
| PQE-BE-43 export pipeline failure → `update_status_failed` | A (deferred) | Side-effect of a Celery worker task (`export_xlsx_task`). Unit-mocking `update_status_failed` at this level couples to Celery internals not exposed in the service boundary. | Wave B: mock `export_xlsx_task` at the Celery task boundary and assert `update_status_failed` is called when `XlsxBuildError` is raised. |
