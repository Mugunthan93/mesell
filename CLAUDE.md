...
4. **No agent writes to another agent's memory.** Memory directories are owned. If you need info that's not yet recorded in another agent's memory, escalate via the STATUS file blocker mechanism.
   - *Persistence rules: launch sessions as `mugunthansrinivasan` (not root/sudo), and isolated/dispatched agents persist shared memory via git-plumbing/Bash to the main tree (not Edit/Write). See `docs/AGENT_MEMORY_PERSISTENCE.md`.*
...
8. **Task Completion Protocol (founder-ruled 2026-06-22).** A task is NOT done until two conditions hold. Full mechanics + command flow live in the canonical skill `.claude/skills/meesell-task-completion-protocol/SKILL.md` (also reflected in `docs/GIT_WORKFLOW.md` and `docs/dev/ENV_MANAGER.md`); the two rules are:
   - **(A) Persist-on-finish (commit + push artifacts).** Durable outputs — `.claude/agent-memory/**`, `docs/status/STATUS_*.md`, `docs/status/feature_board_*.md`, and any spec/doc the task produced — must be **committed AND pushed to the remote**, never left as uncommitted master-tree dirt. Worktree agents commit them to the feature branch so they ride the PR; fast-mode coordinators / standalone agents push them via a dedicated `chore/<slug>-scribe` branch + PR or the git-plumbing route for write-protected `.claude/` files; isolated builders that cannot write their own memory REPORT learnings and the dispatching coordinator scribes AND pushes them. "Definition of Done" includes "the artifacts are on the remote," not merely edited locally.
   - **(B) Rebuild-localhost-on-merge.** Every merge to `develop` MUST be followed by an affected-scope localhost rebuild so the running dev stack matches `develop`: pull develop into the baseline checkout, rebuild the affected scope (`python3 tools/meesell_env.py baseline refresh` for frontend/federation merges; restart the baseline backend `uvicorn --reload` on `:8000` for backend merges; refresh the `:7700` dev-manager), then verify the affected ports via `python3 tools/meesell_env.py status`. Docs-only merges need no rebuild — scope it sensibly. The merging agent (or the master session that performed the merge) triggers the rebuild.
...
- **Persist-on-finish (Rule A)**: a task is done only when its durable outputs (`.claude/agent-memory/**`, `docs/status/STATUS_*.md`, `docs/status/feature_board_*.md`, produced specs/docs) are **committed AND pushed** — on the feature branch (rides the PR) or via a `chore/<slug>-scribe` PR / git-plumbing for `.claude/`. Never leave them as uncommitted master-tree dirt. See `docs/GIT_WORKFLOW.md` and `.claude/skills/meesell-task-completion-protocol/SKILL.md`.
- **Rebuild-localhost-on-merge (Rule B)**: every merge to `develop` is followed by an affected-scope localhost rebuild so the running dev stack matches `develop` (pull develop → `meesell_env.py baseline refresh` for FE/federation or restart `uvicorn --reload :8000` for backend → verify ports via `meesell_env.py status` → refresh the `:7700` dashboard). Docs-only merges skip the rebuild. See `docs/dev/ENV_MANAGER.md`.
